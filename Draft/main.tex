...
\subsection{Random Matrix Theory: Diagnostics of quantum chaos}\LA{RMT_SECT}
...
\begin{figure}
    \centering
    \subcaptionbox{$\sigma_0=0$\LA{NORMAL_STATIC_a}}
    {
        \begin{tikzpicture}[trim axis left, trim axis right]
            \begin{axis}[
                width=0.44\textwidth,
    ...
    \hspace{1.7cm}
    \subcaptionbox{$\sigma_0=2$\LA{NORMAL_STATIC_b}}
    {
        \begin{tikzpicture}[trim axis left, trim axis right]
            \begin{axis}[
                width=0.44\textwidth,
...
\subsubsection{Level spacing distribution and spectral form factor}\LA{LSD_SFF}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[trim axis left, trim axis right]
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Not averaged histogram of the level spacing distribution of our de Sitter Static Patch Brickwall model using $\sigma_0=$}\LA{NOT_AVERAGE_HISTO}
\end{figure}
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}
            \begin{axis}[width=0.8\textwidth,height=0.8*7/10*\textwidth,
                ...
                y label style = {rotate = -90},
                scaled ticks = false,
                xtick={0, 0.05,0.1,0.15,0.20,0.25},
                minor x tick num = 8,
                minor y tick num = 8]
                \addplot+ [orange!70!white]
                table {etc/data/BETA_SIGMA0_d2_Brody.dat};
                \addplot+ [blue!70!white]
                table {etc/data/BETA_SIGMA0_d3_Brody.dat};
                \addplot+ [green!70!white] 
                table {etc/data/BETA_SIGMA0_d4_Brody.dat};
            \end{axis}
        \end{tikzpicture}

    \caption{Relation between the $\beta$ parameter of RMT and the $\sigma_0$ parameter of our model, where we can observer the transition between the different types of Chaotic Systems (a) and the transition between Chaos and Integrability (b). We have considered $d=2,3,4$}\LA{BETA-SIGMA}
\end{figure}
...
These cases ranges from $\sigma_0=0$, where the level spacing distribution has a delta like behaviour, which could be understood as an \textit{extreme} version of level repulsion and the Wigner-Dyson distribution\footnote{As mentioned in \cite{das2023fuzzballsrandommatrices}}; to $\sigma_0=0.017,0.024,0.030$ where the system shows a GSE, GUE, GOE behaviour respectivley; and ending with $\sigma_0=0.5$ where our system shows a clear Poissonian behaviour. However, it is noticeable to mention that in the Chaotic cases the level repulsion is not absolute, there is an undesirable bump near the horizon of the histograms.
...
\begin{figure}[h]
    ...
    \caption{Level spacing distributions of the normal modes, considering a disorder average. The solid lines are given by their respective analytic distribution}\LA{FIVE-CASES}
\end{figure}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[trim axis left, trim axis right]
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Not averaged histogram of the level spacing distribution of our de Sitter Static Patch Brickwall model using $\sigma_0=$}\LA{SFF-not-averaged}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{Spectral Form Factor of the scalar fields considering a disorder-average}\LA{SFF-Averaged}
\end{figure}
